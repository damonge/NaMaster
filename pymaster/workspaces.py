...
class NmtWorkspaceFlat(object) :
    ...
    def get_ell_sampling(self) :
        ...
        """
        ells=lib.get_ell_sampling_flat(self.wsp,self.wsp.fs.n_ell)
    ...
    def couple_cell(self,ells,cl_in) :
        ...
        :return: coupled power spectrum. The coupled power spectra are returned at the multipoles returned by calling :func:`get_ell_sampling`.
        """
    ...
    def decouple_cell(self,cl_in,cl_bias=None,cl_noise=None) :
        ...
        :param cl_in: set of input power spectra. The number of power spectra must correspond to the spins of the two fields that this NmtWorkspaceFlat object was initialized with (i.e. 1 for two spin-0 fields, 2 for one spin-0 and one spin-2 field and 4 for two spin-2 fields). These power spectra must be defined at the multipoles returned by :func:`get_ell_sampling`.
...
def deprojection_bias_flat(f1,f2,ells,cls_guess) :
    """
    Computes the bias associated to contaminant removal to the cross-pseudo-Cl of two flat-sky fields. The returned power spectrum is defined at the multipoles returned by the method :func:`get_ell_sampling` of the NmtWorkspaceFlat object that would be generated by the combination of f1 and f2.
...
def compute_coupled_cell_flat(f1,f2) :
    """
    Computes the angular power spectra of two masked flat-sky fields (f1 and f2) without aiming to deconvolve the mode-coupling matrix. Effectively, this is equivalent to computing the map FFTs and averaging over rings of wavenumber.  The returned power spectrum is defined at the multipoles returned by the method :func:`get_ell_sampling` of the NmtWorkspaceFlat object that would be generated by the combination of f1 and f2.

    :param NmtFieldFlat f1,f2: fields to correlate
    :return: array of coupled power spectra
    """
    if((f1.fl.nx!=f2.fl.nx) or (f1.fl.ny!=f2.fl.ny)) :
        raise KeyError("Fields must have same resolution")
    
    cl1d=lib.comp_pspec_coupled(f1.fl,f2.fl,f1.fl.nmaps*f2.fl.nmaps*f1.fl.fs.n_ell)
...
def compute_full_master_flat(f1,f2,b,cl_noise=None,cl_guess=None,ells_guess=None,workspace=None,nell_rebin=2) :
    ...
    :param cl_noise: noise bias (i.e. angular power spectrum of masked noise realizations) (optional).  This power spectrum should be defined at the multipoles returned by the method :func:`get_ell_sampling` of the NmtWorkspaceFlat object that would be generated by the combination of f1 and f2.
    :param cl_guess: set of power spectra corresponding to a best-guess of the true power spectra of f1 and f2. Needed only to compute the contaminant cleaning bias (optional).
