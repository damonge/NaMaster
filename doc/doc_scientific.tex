...
\section{Flat-sky}
  ...
  \subsection{Pseudo-$C_\ell$ estimator - discrete formalism}
    ...
    \begin{equation}
      {\bf a}_{\bf k}\equiv\sum_{\bf x}\frac{\Delta{\bf x}^2}{2\pi}\,_{s_a}{\sf E}_{\bf k}({\bf x}){\bf a}_{\bf x},
    \end{equation}
    ...
    At this stage it is natural to connect directly with the final bandpowers. In this case we will define a bandpower ${\sf B}_b$, indexed by an integer $b$ as the average of the covariance above over a set of ${\bf l}$ values $S_b$:
    \begin{equation}
