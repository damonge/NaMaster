...
{\functi{void}{nmt\_compute\_coupled\_cell}{\vari{nmt\_field *}{fl1},\vari{nmt\_field *}{fl2},\vari{double **}{cl\_out}}}
 This computes the full-sky angular cross-power spectrum of two masked fields {\tt fl1} and {\tt fl2} without aiming to deconvolve the mode-coupling matrix. Effectively, this is equivalent as calling the usual HEALPix {\tt anafast} routine on the masked and contaminant-cleaned maps. The coupled power spectrum is returned in {\tt cl\_out}, which should have been previously allocated (see description in the introduction to this section). Since no attempt is made to deconvolve the mode-coupling matrix, this function does not require a {\tt nmt\_workspace} structure.
\end{funcdef}
